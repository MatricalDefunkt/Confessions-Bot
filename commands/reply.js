...
module.exports = {
  ...
  async execute ( interaction, client )
  {
    ...
    if ( action === "unblock" || !action )
    {
      ...
      if ( linkOrId.length === 19 )
      {
        replyId = linkOrId;
        link = `https://discord.com/channels/${ guildId }/${ confessId }/${ linkOrId }`;
      } else
      {
        replyId = linkOrId.substring( 67 )
        link = linkOrId
      }

      if ( link.length !== 86 ) return interaction.editReply( { content: `Please check the link or ID you have provided, as it seems to be incorrect. Here is your reply for your reference:\n----------------\n${ replyText }` } )
